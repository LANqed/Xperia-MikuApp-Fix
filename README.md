...
`sync_bilibili_favorites.py` 可以把 Bilibili 收藏夹同步成播放列表：一个收藏夹对应一个或多个播放列表，只下载音频流和封面，不下载视频画面。
...
7. 写回 `music.json`：保留所有不带 `source = bilibili-favorites` 的播放列表，然后追加本轮生成的列表。
